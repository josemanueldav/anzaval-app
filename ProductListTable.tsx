  ...
  return (
    <div className="w-full">
      ...
      <div className="hidden md:block">
        ...
        <div className="overflow-x-auto border border-white/10 rounded-xl bg-gray/5">
            ...
            <tbody>
              {products.map((p) => (
                <tr
                  ...
                  className="border-t border-gray/400 dark:border-gray/100 hover:bg-white/5 text-gray-400 dark:text-gray-300 cursor-pointer"
                >
                  {cols.map((c) => (
                    <td key={c.key} className="px-4 py-2 text-gray-800 dark:text-gray-200 font-medium whitespace-nowrap">
                      {c.key === "imagenes"
                        ? `${p.imagenes ?? 0} fotos`
                        : p[c.key] ?? "—"}
                    </td>
      ...
      <div className="md:hidden grid grid-cols-1 gap-4 mt-4">
        {products.map((p) => (
          <div
            key={p.id}
            onClick={() => navigate(`/productos/${p.id}`)}
            className="bg-white dark:bg-gray-900 shadow-md border border-gray-100 dark:border-gray-700 rounded-xl p-6"

          >
            <h3 className="text-lg font-semibold mb-1">{p.descripcion_corta}</h3>
            <p className="text-gray-300 text-sm mb-2">Tag: {p.tag ?? "—"}</p>
            <p className="text-gray-400 text-xs">Ubicación: {p.ubicacion ?? "—"}</p>
          </div>
        ))}
      </div>
    </div>
